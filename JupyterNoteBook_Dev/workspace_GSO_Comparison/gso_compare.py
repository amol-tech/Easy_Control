...
def get_lookup_part_dict(tree, dict_lookups):
    ...
    for sub_element in tree.findall('.//dataGroup/part'):
        ...
        if ('beRelationId' in dict_elm):
            dict_lookup[dict_elm['id']] = 'REL-' + dict_lookups['relation'][dict_elm['beRelationId']] if dict_elm['beRelationId'] in dict_lookups['relation'] else 'REL-'+ dict_elm['beRelationId']
        elif ('dataGroupId' in dict_elm):
            dict_lookup[dict_elm['id']] = 'DG-' + dict_lookups['dataGroup'][dict_elm['dataGroupId']] if dict_elm['dataGroupId'] in dict_lookups['dataGroup'] else 'DG-'+ dict_elm['dataGroupId']
        elif ('fieldId' in dict_elm):
            dict_lookup[dict_elm['id']] = 'FIELD-' + dict_lookups['field'][dict_elm['fieldId']] if dict_elm['fieldId'] in dict_lookups['field'] else 'FIELD-'+ dict_elm['fieldId']
    return dict_lookup
...
# Verify only mapping
def compare_mapping(dict_result, key):
...
def compare_element(dict_config, tree_elm_cs, tree_elm_gs, dict_parent=None):
    global dict_configs
    ...
    compare_addon = dict_config['compare_addon'] if 'compare_addon' in dict_config else None
    ...
    for key in dict_cs_elm_id:
        ...
        if (key in dict_gs_elm_id.keys()):
            ...
            # Addon Verification
            if (compare_addon != None and compare_addon != 'None'):
                str_exec = compare_addon + '(dict_result,key)'
                eval(str_exec)
            ...
            if (list_child_element != None):
                for child_elm_name in list_child_element:
                    ...
                    if (len(list_fetch) == 1):
                        ...
                        list_tmp = compare_element(dict_child_config, dict_cs_tree_elm[key], dict_gs_tree_elm[key],
                                                  dict_result)
...
def compare_config_element(dict_config, tree_elm_cs, tree_elm_gs):
    global dict_cs_lookups
    global dict_gs_lookups
    global SHOW_WARN
...
def compare_gso(file_config, file_name_cs, file_name_gs, dir_out,show_warning=True):
    ...
    for dict_config in dict_configs['elements']:
        if (dict_config['parent'] == 'Y'):
            if (dict_config['name'] == 'config'):
                list_result = compare_config_element(dict_config, tree_cs, tree_gs)
            else:
                list_result = compare_element(dict_config, tree_cs, tree_gs)
